...
public class SpringCloudFunctionProvider implements CloudFunctionProvider, ApplicationContextAware {
     ...
     */
    private static final RequestMethod[] functionRequestMethods = new RequestMethod[] { POST };
    ...
    public List<RouterOperation> getRouterOperations(OpenAPI openAPI) {
        ...
        functionCatalogOptional.ifPresent(
                functionCatalog -> {
                    // FIX Function with _registration are not resolved by functionCatalog.lookupqq
                    Set<String> names = functionCatalog.getNames(null).stream().filter(s -> !s.startsWith("&")).map(s -> s.replace("_registration", "")).collect(Collectors.toSet());
                    for (String name : names) {
                        if (!RoutingFunction.FUNCTION_NAME.equals(name) && applicationContext.containsBean(name)) {
                            ...
                            else if (function != null && function.isConsumer()) {
                            ...
                            else if (function != null && function.isSupplier()) {
