        ...
        public Object resolveArgumentValue(MethodParameter parameter,
            ...
            if (message.getHeaders().containsKey(MessageHeaders.CONTENT_TYPE)
                ...
                Flux<Object> argument = data.map(buffer -> {
                    ...
                    // FIX KOMUNE I don't know way but data //(NettyDataBuffer ->  EmptyByteBufBE)
                    // enter in map with an empty buffer while it was not enterring before
                    // I need to check if the buffer is empty or not
                    // if (JsonMapper.isJsonString(bytePayload)) {
                    if (bytePayload.length > 0 && JsonMapper.isJsonString(bytePayload)) {
