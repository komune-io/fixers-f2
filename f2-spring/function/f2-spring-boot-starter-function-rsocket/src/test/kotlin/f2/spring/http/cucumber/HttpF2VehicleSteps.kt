...
class HttpF2VehicleSteps : HttpF2GenericsSteps<Vehicle, Vehicle>("Vehicle: "), En {
	...
	override fun consumer(consumerName: String, msgs: Flow<Vehicle>): Unit = runBlocking {
		F2ClientBuilder.get(F2SpringRSocketCucumberConfig.urlBase(bag)).consumer<Vehicle>(consumerName).invoke(msgs)
	}

	override fun supplier(supplierName: String) = runBlocking {
		F2ClientBuilder.get(F2SpringRSocketCucumberConfig.urlBase(bag)).supplier<Vehicle>(supplierName).invoke().toList()
	}
