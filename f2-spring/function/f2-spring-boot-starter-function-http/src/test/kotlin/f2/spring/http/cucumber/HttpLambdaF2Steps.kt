...
class HttpLambdaF2Steps : LambdaF2Steps(), En {
	...
	override fun function(values: List<String>): List<String> = runBlocking {
		F2ClientBuilder.get("http://localhost:${bag.httpPort}").function("functionF2").invoke(values.asFlow()).toList()
	}

	override fun supplier(): List<String> = runBlocking {
		F2ClientBuilder.get("http://localhost:${bag.httpPort}").supplier("supplierF2").invoke().toList()
	}

	override fun consumer(values: List<String>) = runBlocking {
		F2ClientBuilder.get("http://localhost:${bag.httpPort}").consumer(LambdaF2::consumerF2.name).invoke(values.asFlow())
	}
